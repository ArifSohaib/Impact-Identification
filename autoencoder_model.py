...
def main():
    ...
    tensorboard = TensorBoard(log_dir='./logs',
                          ...
                          write_graph=True,
                          write_images=True)
    history = model.fit(x=train_data+(np.random.rand(train_data.shape[0], train_data.shape[1])/10.0), y=train_data, epochs=nb_epoch, batch_size=batch_size, shuffle=True, verbose=1, callbacks=[checkpointer, tensorboard]).history
